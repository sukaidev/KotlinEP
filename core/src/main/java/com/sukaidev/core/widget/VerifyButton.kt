...
/*
    ...
    带倒计时
 */
class VerifyButton(val mContext: Context, attrs: AttributeSet) : Button(mContext, attrs) {
    ...
    private val countDown = object : Runnable {
        override fun run() {
            ...
            this@VerifyButton.setBackgroundColor(
                ContextCompat.getColor(
                    mContext,
                    R.color.common_disable
                )
            )
            this@VerifyButton.setTextColor(ContextCompat.getColor(mContext, R.color.app_main))
    ...
    fun resetCounter(vararg text: String) {
        ...
        this.setTextColor(ContextCompat.getColor(mContext, R.color.app_main))
        mCount = 60
    }
